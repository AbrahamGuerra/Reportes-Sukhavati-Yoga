...
function buildIngestSig(row) {
  const base = [
    String(row.cupon?.toLowerCase() ?? ''),
    String(row.caducidad ?? ''),
    String(row.condiciones?.toLowerCase() ?? ''),
  ].join('|')
...
function mapRow(raw) {
  ...
  if (r.cupon) r.cupon = r.cupon.toUpperCase()
  return r
}

export async function upsertCouponsRows(rows, { schema='reportes_sukhavati', table='cupones' } = {}) {
  if (!rows?.length) return { inserted: 0, updated: 0 }
  const allowed = ['cupon','condiciones','canjes','estado','caducidad']
...
export async function upsertCouponsFromXlsx(xlsxPathOrBuffer, { sheet='Export', schema='reportes_sukhavati' } = {}) {
  ...
  const expectedHeaders = ['Cupón', 'Condiciones', 'Canjes', 'Estado', 'Caducidad']
  ...
  return upsertCouponsRows(rawRows, { schema, table: 'cupones' })
}
